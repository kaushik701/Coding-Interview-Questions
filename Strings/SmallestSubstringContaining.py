# O(b+s) time | O(b+s) space ,b:len(big string),s:len(small string)
bigString = input('Enter the Big String: ')
smallString = input('Enter the Small String: ')
def SmallestSubstringContaining(bigString,smallString):
	targetCharCounts = getCharCounts(smallString)
	substringBounds = getSubstringBounds(bigString,targetCharCounts)
	return getSubstringFromBounds(bigString,substringBounds)
...
def getSubstringFromBounds(string,bounds):
	start,end = bounds
	if end == float('inf'):
		return ""
